...
| `POST /api/runs/{id}/requirements/{index}/auto-judge/toggle` | Opt a requirement into/out of LLM judgment (`auto_judge`) -- a real, honest placeholder that does not yet gate or unlock anything `devsystem.assistant` can already do. Distinct from `automode` below, not a rename -- see [Requirements, verification, and automode]({{ '/explanation/requirements-and-automode/' | relative_url }}). |
| `POST /api/runs/{id}/requirements/{index}/automode/toggle` | Toggle a requirement's `automode` flag ([issue #31](https://github.com/scimbe/CADS-devsystem/issues/31)'s honest first slice) -- recorded and visible, does not yet drive any actual automatic proposal/bidding/role-fill/iteration behavior. Never touches `auto_judge` or `verified`. See [Requirements, verification, and automode]({{ '/explanation/requirements-and-automode/' | relative_url }}). |
